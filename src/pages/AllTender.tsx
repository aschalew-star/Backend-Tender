...
const AllTender: React.FC = () => {
  ...
  const fetchTenders = useCallback(async (page: number) => {
    ...
    try {
      ...
      console.log("AllTender: Fetched tenders:", newTenders.map(t => ({ id: t.id, title: t.title }))); // Debug log
  ...
  const fetchInitialData = useCallback(async () => {
    try {
      const [categoriesRes, regionsRes, adsRes] = await Promise.all([
        axios.get(`${API_BASE_URL}/catagory/all?page=1&limit=100`),
  ...
  const handleViewTenderDetails = useCallback((tender: Tender) => {
    console.log("AllTender: handleViewTenderDetails called with tender:", { id: tender.id, title: tender.title }); // Debug log
    if (!tender?.id) {
    ...
    return (
      <div className="flex justify-center items-center space-x-2 mt-8">
        <button
          ...
          className="px-4 py-2 bg-indigo-600 text-white rounded-md disabled:bg-gray-300 disabled:cursor-not-allowed hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-300"
        >
          Previous
        </button>
        ...
        <main className="flex-1 overflow-y-auto p-4 md:p-8 space-y-8 relative">
          {isLoading && (
            ...
            >
              <div className="flex items-center space-x-2">
                ...
                <span className="text-gray-600">Loading tenders...</span>
              </div>
            </motion.div>
          )}
          <button
          ...
          <AnimatePresence>
            {!isLoading && tenders.length === 0 ? (
              ...
              >
                No tenders available for this page. Try adjusting your filters or navigating to another page.
              </motion.div>
            ) : (
              <TenderGrid
                tenders={tenders}
