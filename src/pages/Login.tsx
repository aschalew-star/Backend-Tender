...
import { Mail, Lock, Eye, EyeOff, AlertCircle, LogIn, LogOut } from 'lucide-react';
import { login, logout, clearError } from '../Redux/authSlice';
...
interface AuthState {
  user: null | { id: string; email: string; role: 'customer' | 'systemUser' };
...
const Login: React.FC = () => {
  ...
  const { user, status, error } = useSelector((state: { auth: AuthState }) => state.auth);
    ...
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-indigo-500 via-purple-500 to-pink-500 p-4">
      <div className="bg-white rounded-2xl shadow-xl p-8 w-full max-w-md transform transition-all duration-300 hover:scale-105">
        ...
        <p className="text-gray-600 text-center mb-6">
          {user ? 'You are logged in' : 'Sign in to access your tender dashboard'}
        </p>
        ...
        {!user ? (
          ...
          <div className="flex justify-center">
            <button
              ...
              className="flex items-center justify-center w-32 py-2 rounded-lg font-semibold text-white bg-indigo-600 hover:bg-indigo-700 shadow-md hover:shadow-lg transition-all duration-200"
            >
        ...
        {error && (
          <div
            className="mt-6 p-4 rounded-lg border-l-4 bg-red-100 border-red-500 transition-all duration-300"
          >
