...
// Main component for editing a tender
export function EditTender() {
  ...
  const [formData, setFormData] = useState({
    ...
    biddingOpen: tender?.biddingOpen || "",
    biddingClosed: tender?.biddingClosed || "",
    categoryName: tender?.category || "",
    subcategoryName: tender?.subcategory || "",
    type: tender?.type || "FREE",
  });
  const [tenderDocs, setTenderDocs] = useState<TenderDoc[]>(tender?.tenderDocs || []);
  const [biddingDocs, setBiddingDocs] = useState<BiddingDoc[]>(tender?.biddingDocs || []);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const validTenderDocs = tenderDocs
        .filter(
          (doc) => doc.name.trim() && doc.title.trim() && doc.type
        )
        .map(({ file, ...doc }) => doc);
      ...
      if (validTenderDocs.length > 0) {
        ...
        tenderDocs.forEach((doc) => {
          if (doc.file && doc.file instanceof window.File) {
            formDataToSend.append('tenderDocsFiles', doc.file);
          }
        });
      }
      ...
      if (validBiddingDocs.length > 0) {
        ...
        biddingDocs.forEach((doc) => {
          if (doc.file && doc.file instanceof window.File) {
            formDataToSend.append('biddingDocsFiles', doc.file);
          }
        });
      }
      ...
      const response = await axios.patch(`http://localhost:4000/api/tender/${id}`, formDataToSend, {
        headers: {
      ...
      navigate('/tenders'); // Navigate back to tenders list
    } catch (error: any) {
      console.error("Updating tender error:", {
      ...
      const errorMessage = error.response?.data?.message?.includes("Invalid file type")
        ...
        : error.response?.data?.message || "Failed to update tender. Please check your inputs and try again.";
  ...
  return (
    <div className="min-h-screen p-4 lg:p-8 font-['Inter'] md:mt-10 mt-5 rounded-2xl">
      <div className="">
            ...
            <div className="flex items-center space-x-4">
              ...
              <div>
                ...
                <p className="text-gray-600 text-sm mt-1">
                  Update the details of the tender opportunity
                </p>
              </div>
            </div>
          </div>
...
// Wrap EditTender with ErrorBoundary
